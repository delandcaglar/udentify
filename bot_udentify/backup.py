...
def start_writing_on_docx(firma, magza_statik_dosya_location_ismi, magaza_id_no, ilk_tarih, son_tarih,
                          magaza_id_yogunluk, performans_tablosu_id):
    global global_test
    ...
    edinme_hunisi_yuzde = request1.main_2_8_edinme_hunisi ( performans_tablosu_listesi[4], ilk_tarih,
    ...
    document.add_paragraph (
        f'{ortalamanin_ustundeki_yogunluklar[0].title ()} tarihi yoğunluğun en çok arttığı tarihlerden biridir. Yoğunluktaki artışın sebebi, kişi sayısındaki artıştır.',
        style='List Bullet'
    )

    document.add_paragraph (
        f'Yoğunluğun en çok artış gösterdiği diğer bir tarih {ortalamanin_ustundeki_yogunluklar[0].title ()}’dir. O gün yoğunluğun artmasının sebebi, mağaza içerisinde geçirilen sürenin artış göstermesidir.',
        style='List Bullet'
    )
    ...
    document.add_paragraph (
        f'En çok ziyaret edilen alan günde ortalama {performans_tablosu_listesi[2]} kişi ile {performans_tablosu_listesi[0].title ()} alanı olmuştur.',
        style='List Bullet'
    )
    ...
    p = document.add_paragraph (
        f"Mağaza içerisindeki en yoğun alan olan {performans_tablosu_listesi[0].title ()} alanının detayına inildiğinde;" )
    ...
    last_paragraph.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p = document.add_paragraph (
        f"({performans_tablosu_listesi[0].title ()} Alanı Toplam Kişi Ve Günlük Süre Grafiği)" )
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    ...
    last_paragraph.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p = document.add_paragraph (
        f"({performans_tablosu_listesi[0].title ()} Alanı Toplam Kişi Ve Saatlik Süre Grafiği)" )
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    ...
    last_paragraph.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p = document.add_paragraph (
        f"({performans_tablosu_listesi[0].title ()} Alanı Yoğunluk Ve Yoğunluk Ortalaması Grafiği)" )
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    ...
    p = document.add_paragraph (
        f"{performans_tablosu_listesi[0].title ()} alanının performans değişimi incelendiğinde;" )
    ...
    last_paragraph.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p = document.add_paragraph (
        f"({performans_tablosu_listesi[0].title ()} Alanı Yüzdelik Performans Değişimi Grafiği)" )
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    ...
    def populate_karsilastirma(kategori_karsilasmasi_1, kategori_karsilasmasi_2):
        ...
        document.add_picture ( os.path.join ( BASE_DIR, f"{magza_statik_dosya_location}/tablo-4.png" ),
                               width=Inches ( 6 ), height=Inches ( 0.75 ) )
        last_paragraph = document.paragraphs[-1]  # resimleri ortalamak icin
        last_paragraph.alignment = WD_ALIGN_PARAGRAPH.CENTER
        p = document.add_paragraph (
            f"({kategori_karsilasmasi_1} ve {kategori_karsilasmasi_2} Alanlarının Yoğunlukları)" )
        p.alignment = WD_ALIGN_PARAGRAPH.CENTER

        p = document.add_paragraph ( f"" )
        p.add_run (
            f'Mağaza içerisindeki {kategori_karsilasmasi_1} ve {kategori_karsilasmasi_1} alanlarının yoğunlukları, metre kareleri ve metre kare başına düşen yoğunluk yüzdeleri kıyaslanmıştır.' ).italic = True

        document.add_paragraph (
            f'Mağaza içerisinde her iki alana ayrılan metre kareler yaklaşık olarak eşittir.', style='List Bullet'
        )
        document.add_paragraph (
            f'{kategori_karsilasmasi_2} alanı, {kategori_karsilasmasi_1} run alanının yaklaşık olarak 1.5 katı yoğunluğa sahiptir.',
            style='List Bullet'
        )
        document.add_paragraph (
        ...
        document.add_paragraph (
            f'Metre kare başına düşen yoğunluklar incelendiğinde, {kategori_karsilasmasi_2} alanının metre kare başına düşen yoğunluğu, {kategori_karsilasmasi_1}’dan daha büyüktür ve bu oran 1’in üzerindedir.',
            style='List Bullet'
        )
        document.add_paragraph (
            f'Yani 1 birim metre kareye 1.15 birim yoğunluk düşmektedir. Alan yoğunluk bakımından verimli kullanılmıştır.',
            style='List Bullet'
        )
        document.add_paragraph (
            f'{kategori_karsilasmasi_2} alanında yoğunluk/metre kare oranı 1’den küçüktür. Buna göre bu alan, yoğunluk bazında verimli kullanılamamıştır. Alana ayrılan metre karenin büyük geldiği düşünülüyorsa alan kademeli olarak küçültülmeli, metre kare küçülürken satış azalmıyorsa alan küçültülmelidir.',
            style='List Bullet'
        )
        document.add_paragraph (
            f'Her iki alanda da yoğunluğu ve dolayısıyla satışları arttırmak için alana gelen kişi sayısını ve geçirilen süreyi arttırmak için aksiyon alınmalıdır.',
            style='List Bullet'
        )
        document.add_paragraph (
        ...
        last_paragraph = document.paragraphs[-1]  # resimleri ortalamak icin
        last_paragraph.alignment = WD_ALIGN_PARAGRAPH.CENTER
        p = document.add_paragraph ( f"({kategori_karsilasmasi_2} ve {kategori_karsilasmasi_1} Tablosu)" )
        p.alignment = WD_ALIGN_PARAGRAPH.CENTER

        p = document.add_paragraph ( f"Detaya inildiğinde;" )
        document.add_paragraph (
            f'{kategori_karsilasmasi_1} alanında yoğunluğu oluşturan asıl parametre kişi sayısıdır. Bu alanda yoğunluğu arttırmak için süreyi arttırmaya odaklanmak gerekir. Bu alanda geçirilen ortalama süre sadece 4.9 saniyedir.',
            style='List Bullet'
        )

        document.add_paragraph (
            f'{kategori_karsilasmasi_2} alanında ise yoğunluk, kişi değil süre ağırlıklıdır. Bu alanda geçirilen süre ortalama 16 saniyedir. Yoğunluğu arttırmak için kişi sayısı artarken sürenin düşmemesine dikkat etmek gerekir. Kişi sayısını arttırmanın satışları da arttırması beklenir.',
            style='List Bullet'
        )
        # document.add_page_break ()
    ...
    last_paragraph = document.paragraphs[-1]  # resimleri ortalamak icin
    last_paragraph.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p = document.add_paragraph ( f"(Boys's and Girl's Tablosu)" )
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
...
magaza_adi_listesi = [["Under Armour", "Akasya", 240, "Under Armour Akasya", 240],
                      ["Under Armour", "Zorlu Center", 239, "Under Armour Zorlu Center", 240],
                      ["Under Armour", "İstinye Park", 228, "Under Armour Istinye Park", 240]]

start_writing_on_docx ( firma, magza_statik_dosya_location_ismi, 240, ilk_tarih, son_tarih, 161,
                        240 )  ##birincisi firma adi ile dosya konumunu bulmaya yariyor   ,ikincisi statik dosya locationlarini bulmaya yariyor
